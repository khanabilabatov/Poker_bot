...
PREFLOP_BB_DEFEND_EQUITY      = 0.38   # BB defends vs standard raise if equity >= this
PREFLOP_SB_COMPLETE_EQUITY    = 0.42   # SB completes for half BB if equity >= this
...
# C-bet frequencies — bet as preflop aggressor on dry/medium boards only.
# Wet boards skipped: lighter WET_BOARD_EHS_PENALTY (0.03) already keeps
# EHS higher on wet boards, and c-betting wet boards risks building big pots
# with marginal hands against draws. Dry/medium boards are safe and profitable.
CBET_PROB_DRY    = 0.70   # dry board: high fold equity, bet frequently
CBET_PROB_MEDIUM = 0.55   # medium board: moderate fold equity

# ── Option B: Kicker quality penalty ──────────────────────────────────────────
# Data analysis showed one pair is -204,859 chips net despite 50% win rate.
# Root cause: we call large bets with weak-kicker top pair (K6, J3, Q9 etc)
# while opponent has same pair with better kicker and keeps betting.
# MC simulation cannot detect this — it randomises opponent hands.
#
# Fix: reduce EHS when we have top pair with a weak kicker.
# Graded penalty so KJ/KQ are unaffected, K9/K8/K6 are penalised.
#
# Kicker < J  (T, 9, 8, 7, 6, 5, 4, 3, 2) → EHS -= 0.06
# Kicker < 8  (7, 6, 5, 4, 3, 2)          → EHS -= 0.11  (replaces the 0.06)
# If we didn't pair the TOP board card, penalty is halved (less certain situation)
KICKER_WEAK_PENALTY      = 0.06
KICKER_VERY_WEAK_PENALTY = 0.11
...
def make_postflop_raise(amount_owed, pot, street,
    ...
    if amount_owed > 0:
        ...
        amount = int(amount_owed * (base + wet_adj * 0.3))
    else:
        base = {"flop": 0.55, "turn": 0.65, "river": 0.65}.get(street, 0.60)
        amount = int(pot * (base + wet_adj))
...
def estimate_fold_equity(game_state, texture, active_opponents):
    ...
    # Use amount_owed to detect a live bet against us — more reliable than
    # parsing street boundaries via get_street_actions() which can miscount.
    if game_state.get("amount_owed", 0) > 0:
        fold_equity -= 0.06
    # Scan recent actions for folds only — safe regardless of street boundary
    my_seat = game_state["seat_to_act"]
    for a in game_state["action_log"][-8:]:
        if a.get("seat") != my_seat and a.get("action") == "fold":
            fold_equity += 0.04
...
def assess_kicker_penalty(my_cards, board_cards):
    """
    Detect weak-kicker one-pair situations and return an EHS penalty.

    When we pair the board with one hole card, our other hole card is the
    kicker. K6 on a K-high board = top pair with a 6 kicker. If the opponent
    also has a K, only the kicker decides the winner — and MC simulation
    cannot detect this because it randomises opponent hands.

    Returns a penalty value (0.0 if no kicker problem detected).

    No penalty when:
    - Both hole cards pair the board (two pair — kicker doesn't matter)
    - Pocket pair (no kicker issue)
    - Neither hole card pairs the board (no pair made)
    - Board has no high card (T or above) — low-board kicker battles rare
    - Kicker is J or above (J, Q, K, A — strong kicker, likely ahead)

    Penalty tiers (halved if we didn't pair the TOP board card):
    - Kicker rank < 8  → KICKER_VERY_WEAK_PENALTY (0.11)
    - Kicker rank < J  → KICKER_WEAK_PENALTY (0.06)
    """
    RANK        = "23456789TJQKA"
    board_ranks = [c[0] for c in board_cards]
    hole_ranks  = [c[0] for c in my_cards]

    # Both hole cards pair the board → two pair, no kicker problem
    if all(r in board_ranks for r in hole_ranks):
        return 0.0
    ...
    # Exactly one hole card pairs the board
    paired_hole   = [r for r in hole_ranks if r in board_ranks]
    unpaired_hole = [r for r in hole_ranks if r not in board_ranks]

    if len(paired_hole) != 1 or len(unpaired_hole) != 1:
        return 0.0   # didn't pair the board, or both paired

    kicker_rank = RANK.index(unpaired_hole[0])
    paired_rank = RANK.index(paired_hole[0])

    # Only apply penalty when there's a high card on the board (T+)
    # Low boards (7-5-2) — kicker battles less common and less costly
    if not any(RANK.index(r) >= RANK.index('T') for r in board_ranks):
        return 0.0

    # Half the penalty if we didn't pair the highest board card
    # (less likely to be dominated — opponent pairing top card is most dangerous)
    highest_board = max(RANK.index(r) for r in board_ranks)
    factor = 1.0 if paired_rank >= highest_board - 1 else 0.5

    if kicker_rank < RANK.index('8'):
        return KICKER_VERY_WEAK_PENALTY * factor   # e.g. K6, J3, Q4
    elif kicker_rank < RANK.index('J'):
        return KICKER_WEAK_PENALTY * factor         # e.g. K9, K8, Q9, JT

    return 0.0   # kicker is J or above — no penalty
...
def was_preflop_aggressor(game_state):
    """
    Returns True if we raised preflop — used to trigger c-bets on the flop.
    A c-bet represents continued strength after preflop aggression and takes
    pots on boards where opponents missed.
    """
    my_seat = game_state["seat_to_act"]
    return any(
        a["seat"] == my_seat and a["action"] in ("raise", "all_in")
        for a in game_state["action_log"]
    )
...
def decide(game_state: dict) -> dict:
    ...
    if street == "preflop":
        ...
        # Identify blind seats for BB defence and SB completion logic
        my_seat   = game_state["seat_to_act"]
        bb_seat   = next((a["seat"] for a in game_state["action_log"] if a["action"] == "big_blind"), None)
        sb_seat   = next((a["seat"] for a in game_state["action_log"] if a["action"] == "small_blind"), None)
        ...
        if is_medium:
            ...
            # BB defence: already invested 1 BB, getting a pot-odds discount.
            # Call a single standard raise if equity >= 0.38.
            # Only applies to standard opens (shove case handled above).
            if (my_seat == bb_seat
                    and n_raises == 1
                    and equity >= PREFLOP_BB_DEFEND_EQUITY
                    and shove_fraction <= 0.25):
                return {"action": "call"}

            # SB completion: complete for half a BB when nobody raised.
            # Profitable with any hand equity >= 0.42 — cheap flop opportunity.
            if (my_seat == sb_seat
                    and n_raises == 0
                    and amount_owed > 0
                    and equity >= PREFLOP_SB_COMPLETE_EQUITY
                    and shove_fraction <= 0.10):
                return {"action": "call"}
        ...
        if is_playable:
            if amount_owed > 0 and shove_fraction > 0.20:
                return {"action": "fold"}
            # SB completion also applies to lower-equity playable hands
            # if completing costs only half a BB (no raise facing)
            if (my_seat == sb_seat
                    and n_raises == 0
                    and amount_owed > 0
                    and equity >= PREFLOP_SB_COMPLETE_EQUITY
    ...
    # ── Option B: kicker quality penalty ──────────────────────────────────────
    # Applied on flop, turn, and river when we have top pair with a weak kicker.
    # Prevents over-committing chips with K6, J3, Q9 type hands.
    # Does not fire for two pair, pocket pairs, flush draws, or low boards.
    if len(board_cards) >= 3:
        kicker_pen = assess_kicker_penalty(my_cards, board_cards)
        if kicker_pen > 0:
            ehs = max(0.0, ehs - kicker_pen)
    ...
    if street == "flop":
        # C-bet: bet as preflop aggressor on dry/medium boards only.
        # Wet boards skipped — see CBET_PROB constants for reasoning.
        if (can_check
                and was_preflop_aggressor(game_state)
                and active_opponents <= 2
                and texture["board_type"] in ("dry", "medium")
                and ehs >= EHS_DRAW):
            cbet_prob = CBET_PROB_DRY if texture["board_type"] == "dry" else CBET_PROB_MEDIUM
            if random.random() < cbet_prob:
                return {"action": "raise", "amount": make_postflop_raise(
                    0, pot, "flop", my_stack, my_bet_this_street, min_raise, texture
                )}
